...
FOLDER_SAVE = './downloads/Wayang Kulit Series/02. Ki Nartosabdo - Wirata Parwa'
    ...
    def y2mate():
        try:
            ...
            for _, source_url in enumerate(urls):
                ...
                download_save = f"{FOLDER_SAVE}/{filename}"
                if os.path.exists(download_save):
                    if REPLACE_ON_EXISTS:
                        os.remove(download_save)
                    else:
                        download_save = f"{FOLDER_SAVE}/{token_urlsafe(6)}_{filename}"
